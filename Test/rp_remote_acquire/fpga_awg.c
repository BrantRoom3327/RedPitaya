...
int fpga_awg_init(void)
{
    ...
    page_ptr = mmap(NULL, AWG_BASE_SIZE, PROT_READ | PROT_WRITE, MAP_SHARED, g_awg_fd, page_addr);
    ...
    g_awg_cha_mem = (uint32_t *)g_awg_reg + (AWG_CHA_OFFSET / sizeof(uint32_t));
    g_awg_chb_mem = (uint32_t *)g_awg_reg + (AWG_CHB_OFFSET / sizeof(uint32_t));
